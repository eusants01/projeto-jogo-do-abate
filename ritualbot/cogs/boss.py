...
class Boss(commands.Cog):
    ...
    async def boss(self, ctx, *, nome=None):
        ...
        if BOSS_ATIVO is not None:
            await ctx.send("⚠️ Já existe um **boss ativo** no servidor.\nDerrote ou aguarde ele acabar antes de invocar outro.", delete_after=12)
            return
    ...
    async def limpar_boss(self, ctx):
        ...
        try:
            ...
            pass
        BOSS_ATIVO = None
        await ctx.send("🧹 Boss ativo limpo manualmente. Agora outro boss pode ser invocado.", delete_after=10)
